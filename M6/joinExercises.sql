...
l.ClientId, 
l.EmailAddress 
FROM Client c LEFT OUTER JOIN Login l ON c.ClientId= l.ClientId;
...
-- nope :(
--------------------
SELECT c.FirstName, c.LastName, l.EmailAddress, l.ClientId FROM Client c LEFT OUTER JOIN login l ON c.ClientId = l.ClientId WHERE c.FirstName = 'Romeo' AND c.FirstName = 'Seaward';
...
e.Name Category
FROM ExerciseCategory e INNER JOIN ExerciseCategory p ON e.ParentCategoryId = p.ExerciseCategoryId;
...
SELECT p.Name ParentCategory,
e.Name Category
FROM ExerciseCategory e LEFT OUTER JOIN ExerciseCategory p ON e.ParentCategoryId = p.ExerciseCategoryId;
...
-- 26 Workouts, 3 Goals
--------------------
SELECT w.Name, g.Name GoalName 
FROM Workout w 
LEFT OUTER JOIN WorkoutGoal wg ON w.WorkoutId = wg.WorkoutId AND wg.GoalId = 10 
wLEFT OUTER JOIN Goal g ON wg.GoalId = wg.GoalId;
...
--------------------
SELECT e.Name,                                                                                                                                          eiuv.Value,
u.Name UnitName
FROM Exercise e INNER JOIN ExerciseInstance ei ON ei.ExerciseId = e.ExerciseId
